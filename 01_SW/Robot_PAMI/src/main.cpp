...
differentiel Robot(&StepperG, &StepperD, &StopMove);
    ...
    while (true)
    {
        if (FsmState == GAME)
        {
            ...
            else
            {
                ...
                StopMove = (distance < 150 and temps > 85) ? 1 : 0;
                LedR = StopMove;
                LedG = StopMove;
                LedB = StopMove;
            }
...
//Position team bleu
    ...
    Robot.setPosition(120, 1843, 90, Couleur_Team);
    Robot.Robotgoto(450, 1840, 180, Couleur_Team, NORMALSPEED);  // Step 1
    while (temps < 15 && !end_match && !LatchBau)
    {
        ThisThread::sleep_for(200ms);
    }
    Robot.Robotgoto(450, 1700, 180, Couleur_Team, NORMALSPEED);  // Step 2
    while (temps < 85 && !end_match && !LatchBau)
    {
        ThisThread::sleep_for(200ms);
    }
    Robot.Robotgoto(818, 892, 180, Couleur_Team, NORMALSPEED);  // Step 3
    ...
    Robot.setPosition(120, 1743, 90, Couleur_Team);
    while (temps < 10 && !end_match && !LatchBau)
    {
        ThisThread::sleep_for(200ms);
    }
    Robot.Robotgoto(320, 1740, 180, Couleur_Team, NORMALSPEED);  // Step 1
    Robot.Robotgoto(320, 1700, 180, Couleur_Team, NORMALSPEED);  // Step 2
    while (temps < 85 && !end_match && !LatchBau)
    {
        ThisThread::sleep_for(200ms);
    }
    Robot.Robotgoto(752, 175, 0, Couleur_Team, NORMALSPEED);  // Step 3
...
Robot.setPosition(120, 1600, 90, Couleur_Team);
  while (temps < 85 && !end_match && !LatchBau)
    {
        ThisThread::sleep_for(200ms);
    }
Robot.Robotgoto(128, 957, 180, Couleur_Team, NORMALSPEED);  // Step 1
...
void main_thread(void)
{
    ...
    while (1)
    {
        ...
        case IDLE:
            ...
            {
                endMatch_thread.start(TimeProcess);
                FsmState = WAIT_START;
            }
            break;
