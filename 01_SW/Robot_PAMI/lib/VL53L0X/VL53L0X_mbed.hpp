...
public:
    ...
    explicit VL53L0X_mbed(I2C& i2c, uint8_t address7bit = 0x29);

    void setAddress(uint8_t new_addr_7bit);
    ...
    void setTimeout(uint16_t timeout_ms) { io_timeout = timeout_ms; }
    uint16_t getTimeout() const { return io_timeout; }
    bool timeoutOccurred();

    uint8_t getLastStatus() const { return last_status; }
...
private:
    ...
    static uint8_t encodeVcselPeriod(uint8_t period_pclks);
};