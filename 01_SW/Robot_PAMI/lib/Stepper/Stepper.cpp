...
void Stepper::move(int steps)
{
    ...
    {   
        _dirPin = (_inv) ? CW : CCW; //set output pin direction value (reverse the physical direction)
        _dir = CCW;                   //set direction to logic 
        _steps = -steps;    //total steps per move must be unsigned
    }
    else
    {

        _dirPin = (_inv) ? CCW : CW; //set output pin direction value (reverse the physical direction)
        _dir = CW;                    //set direction to logic 
