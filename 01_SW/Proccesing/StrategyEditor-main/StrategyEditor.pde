...
void drawTerrain() {
  ...
  terrainView.translate(viewOffsetX, viewOffsetY);
  terrainView.scale(mmToPx);

  if (terrainImage != null) {
    terrainView.image(terrainImage, 0, 0, TERRAIN_W_MM, TERRAIN_H_MM);
  } else {
    terrainView.fill(0);
...
void mouseWheel(processing.event.MouseEvent event) {
  ...
  float worldX = screenToMmX(mouseX);
  float worldY = screenToMmY(mouseY);
...
int getSegmentIndexUnderMouse() {
  ...
  float mx = mouseX - viewOffsetX;
  float my = mouseY - viewOffsetY;
